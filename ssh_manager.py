...
class SSHManager:
    ...
    def _prepare_command(self, command: str, config: Dict) -> Tuple[str, Optional[str]]:
        ...
        if user != 'root' and command.startswith('sudo ') and password:
            ...
            # Synology/DSM Path Fix: use 'env' to set PATH and sh -c to ensure it applies to the whole command string
            # shlex.quote ensures the command is passed safely to sh -c
            command_to_run = f'sudo -S -E env PATH="$PATH:{path_list}" sh -c {shlex.quote(cmd_body)}'
            return command_to_run, password
        else:
            # For non-sudo or root, still use env and sh -c for PATH consistency across entire command string
            command_to_run = f'env PATH="$PATH:{path_list}" sh -c {shlex.quote(command)}'
