...
class TestSSHManagerLogic(unittest.TestCase):
    ...
    def test_prepare_command_non_root_sudo(self):
        ...
        self.assertIn('sudo -S -E env PATH="$PATH:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin" sh -c', prepared_cmd)
        self.assertIn("'apt update'", prepared_cmd)
    ...
    def test_prepare_command_non_root_no_sudo(self):
        ...
        self.assertIn('env PATH="$PATH:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin" sh -c', prepared_cmd)
        self.assertIn("'ls -la'", prepared_cmd)
    ...
    def test_prepare_command_root(self):
        ...
        self.assertIsNone(password)
        self.assertIn('env PATH="$PATH:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin" sh -c', prepared_cmd)
        self.assertIn("'sudo apt update'", prepared_cmd)
    ...
    def test_execute_internal_success(self, mock_client_class):
        ...
        config = self.servers[0]
        # Command without spaces: shlex.quote("ls") -> "ls"
        ...
        self.assertIn("sh -c ls", args[0])

    @patch('paramiko.SSHClient')
    def test_execute_internal_with_spaces(self, mock_client_class):
        mock_client = mock_client_class.return_value
        mock_stdout = MagicMock()
        ...
        config = self.servers[0]
        # Command with spaces: shlex.quote("ls -la") -> "'ls -la'"
        self.manager._execute_internal(mock_client, config, "ls -la", "server1")

        args, _ = mock_client.exec_command.call_args
        self.assertIn("sh -c 'ls -la'", args[0])
